...
Phantom's memory. It holds the watchlist, keeps the dedupe ledger, mints
...
would be fatal if the Hub were doing the watching — it isn't. Phantom runs
...
`INGEST_TOKEN` is a long random string you invent — it is what Phantom
presents:
...
| `POST` | `/observations` | token | Phantom reports what it saw |
| `GET` | `/watchlist` | token | `products` to poll, and `sources` to hunt in |
| `POST` | `/ingest` | token | Phantom posts what it saw |
...
`BLOCKED` belongs to Phantom. Right now that is all of them, which is why
...
cheque; it is refused where a person sets it, not only where Phantom reads
it.
...
## Two tables for what Phantom sees
...
switching Phantom on announces everything at once — the same mistake the
...
src/auth.ts        Bearer token for Phantom, signed cookie for the browser
