...
test('the page says what still wakes it', async () => {
  ...
  assert.match(text, /releasing today/);
});
...
test('THE PRODUCT NAME OPENS THE PRODUCT PANEL, NOT THE SHOP', async () => {
  // It was a plain div sitting beside an Open button, so clicking the obvious
  // thing — the name of the thing you want — selected the text.
  //
  // The first fix made it a link to the retailer, which was the wrong
  // destination: Open already goes there, so that was two controls pointing
  // at one website while the in-app panel stayed unreachable from the card
  // that names the product. The name opens the panel.
  const d = JSON.parse(JSON.stringify(DASHBOARD));
  d.missions = [
    { ...DASHBOARD.missions[0], id: 77, state: 'in', price: 59.99, sellerKind: 'retailer',
      productName: 'Chaos Rising ETB', url: 'https://t.test/etb', enabled: true },
  ];
  const h = await boot(d);

  const name = h.doc.querySelector('#live-list button.nm') as HTMLButtonElement;
  assert.ok(name, 'the name is a button, not text');
  assert.equal(name.type, 'button', 'never a form submit');
  assert.equal(name.getAttribute('href'), null, 'it goes nowhere on the web');
  assert.match(name.textContent!, /Chaos Rising/);
  assert.equal(name.title, 'Chaos Rising ETB', 'the full name still on hover');

  const dialog = $(h, '#detail-dialog') as any;
  assert.equal(dialog.open, false, 'shut to begin with');
  name.click();
  await h.settle();

  assert.equal(dialog.open, true, 'the product panel opened');
  assert.match(dialog.textContent, /Chaos Rising/, 'and it is this product');
});

test('the Open button still goes to the shop', async () => {
  // Two controls, two destinations — which is the point. The name is the
  // app's own view of the product; Open is the retailer's page, where you
  // would actually buy it.
  const d = JSON.parse(JSON.stringify(DASHBOARD));
  d.missions = [
    { ...DASHBOARD.missions[0], id: 78, state: 'in', price: 59.99, sellerKind: 'retailer',
      productName: 'Chaos Rising ETB', url: 'https://t.test/etb', enabled: true },
  ];
  const h = await boot(d);
  const open = h.doc.querySelector('#live-list a.go') as HTMLAnchorElement;
  assert.ok(open, 'Open is still there');
  assert.equal(open.getAttribute('href'), 'https://t.test/etb');
