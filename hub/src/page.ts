...
/* The product name opens the product's own panel — the same pop-up a mission
   card opens, not the retailer's website, which is what the Open button beside
   it is for. It was a plain div, so clicking the obvious thing selected the
   text instead.
   A real <button>, because this opens a dialog rather than going to a URL;
   stripped of every button affordance so it still reads as the name. The whole
   ROW is deliberately not the target: that would swallow the price, make the
   text impossible to select, and nest one click target inside another. */
button.nm { background: none; border: 0; padding: 0; margin: 0; font: inherit;
            text-align: left; width: 100%; cursor: pointer; }
button.nm:hover { color: var(--accent); text-decoration: underline; }
button.nm:focus-visible { outline: 2px solid var(--accent); outline-offset: 2px;
                          border-radius: 3px; }
...
function renderLive() {
  ...
  for (const m of shop.slice(0, 6)) {
    ...
    const g = el('div', 'g');
    // The name opens the PRODUCT'S PANEL, not the shop. The Open button beside
    // it already goes to the retailer, and the first version of this made the
    // name a second route to the same website — two controls, one destination,
    // and the in-app view still unreachable from the panel that names it.
    const nm = el('button', 'nm', shortName(m.productName));
    nm.type = 'button';
    nm.title = m.productName;
    nm.addEventListener('click', () => openDetail('mission', m.id));
    g.appendChild(nm);
    const meta = el('div', 'meta');
